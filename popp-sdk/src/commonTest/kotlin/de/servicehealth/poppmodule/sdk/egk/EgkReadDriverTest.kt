...
class EgkReadDriverTest {
    ...
    private fun driver(transport: FakeTransport, card: FakeCard) =
        EgkReadDriver(transport, card, newSessionId = { sessionId })

    @Test
    fun full_exchange_yields_success_and_sends_expected_messages() = runTest {
        val cmd = "00A4040C"
    ...
    @Test
    fun server_error_yields_failed() = runTest {
    ...
    @Test
    fun status_word_mismatch_throws_protocol_and_closes() = runTest {
        val cmd = "00A4040C"
    ...
    @Test
    fun throwing_transceive_surfaces_as_poppsdkerror_and_closes() = runTest {
        val cmd = "00A4040C"
        val transport = FakeTransport(listOf(scenario(0, listOf(ScenarioStep(cmd, listOf("9000"))))))
    ...
    @Test
    fun progress_is_emitted_per_step() = runTest {
        val transport = FakeTransport(
            listOf(
    ...
    @Test
    fun silent_server_times_out_as_network_and_closes() = runTest {
        ...
        var closed = false
        val transport = object : PoppServiceTransport {
        ...
        val error = assertFailsWith<PoppSdkError.Network> {
