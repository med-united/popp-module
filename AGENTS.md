...
- **3rd-party demo** under `popp-demo/popp-3rd-party-app-demo/`:
  - `:popp-demo:popp-3rd-party-app-demo:shared3rdPartyApp` -> Compose-MP library; `App.kt` (commonMain, package `…demo`) + `MainViewController.kt` (iosMain, package `…demo.thirdparty`); depends on `projects.poppDemo.shared`; static iOS framework `Shared3rdPartyApp`. Namespace `…demo.thirdparty.shared`.
...
- `:popp-demo:popp-3rd-party-app-demo:shared3rdPartyApp` — `commonMain`, `androidMain`, `iosMain` (+ `QrCameraViewfinder.ios.kt`), `androidHostTest`, `commonTest`.
- `:popp-demo:popp-insurance-app-demo:sharedInsuranceApp` — `commonMain`, `androidMain`, `iosMain`.
