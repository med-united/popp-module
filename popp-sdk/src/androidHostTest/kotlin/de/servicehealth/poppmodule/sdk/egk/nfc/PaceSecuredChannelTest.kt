...
class PaceSecuredChannelTest {
    @Test
    fun `establishes PACE lazily and round-trips logical APDUs over secure messaging`() =
        runTest {
    ...
    @Test
    fun `round-trips multi-block payloads over secure messaging`() =
        runTest {
            val card = FakePaceCard(can = "123456")
            val channel = PaceSecuredChannel(card, can = "123456")
    ...
    @Test
    fun `round-trips Le-bearing APDUs over secure messaging`() =
        runTest {
            val card = FakePaceCard(can = "123456")
            val channel = PaceSecuredChannel(card, can = "123456")
    ...
    @Test
    fun `reader without extended-length support fails with a clear error`() =
        runTest {
    ...
    @Test
    fun `oversized secure-messaging APDU fails with a clear error`() =
        runTest {
    ...
    @Test
    fun `wrong CAN surfaces as WRONG_CAN`() =
        runTest {
            val card = FakePaceCard(can = "123456")
    ...
    @Test
    fun `tag loss surfaces as CARD_LOST`() =
        runTest {
            val card = FakePaceCard(can = "123456")
            val channel = PaceSecuredChannel(card, can = "123456")
    ...
    @Test
    fun `corrupted response MAC surfaces as SECURE_CHANNEL_FAILED`() =
        runTest {
            val card = FakePaceCard(can = "123456")
            val channel = PaceSecuredChannel(card, can = "123456")
            channel.transceive("00A4040C") // PACE up
